...
- **左右滑动翻页**：序列 = `front` + `chapters`（与左栏同源），每本书自动获得；触屏设备上页尾显示一行提示
  （`ctl-swipe-hint`，靠 `hover:none` + `pointer:coarse` 判定，桌面端不显示）。
  四重护栏：只认单指；起点在 canvas / input / 代码块 / 抽屉内不触发；距左右边缘 30px 内不触发（避开 iPadOS 边缘返回）；
  位移 >60px 且水平位移 > 垂直位移 2 倍。**没有页尾按钮**，滑动是唯一入口。
